...
from utils.data import read_and_unpkl
from utils.utils import norm_entropy
import numpy as np
import editdistance
import ast
...
def topk_test(model, data, mask, topk=3, need_batch=False, subgraph_loader=None):
    ...
    med_confidence_sorted_conf_mask = index_to_mask(sorted_conf_idx[full_length // 3: full_length * 2 // 3],
                                                    size=full_length)
    ...
    top3_low_acc = torch.sum(com_res[mask & low_confidence_sorted_conf_mask]) / \
                   com_res[mask & low_confidence_sorted_conf_mask].shape[0]
    top3_med_acc = torch.sum(com_res[mask & med_confidence_sorted_conf_mask]) / \
                   com_res[mask & med_confidence_sorted_conf_mask].shape[0]
    top3_high_acc = torch.sum(com_res[mask & high_confidence_sorted_conf_mask]) / \
                    com_res[mask & high_confidence_sorted_conf_mask].shape[0]
    # true_acc = torch.sum(com_res[mask & true_mask]) / com_res[mask & true_mask].shape[0]

    res = data.y.view(-1).eq(r_y_pred)
    top1_low_acc = torch.sum(res[mask & low_confidence_sorted_conf_mask]) / \
                   res[mask & low_confidence_sorted_conf_mask].shape[0]
    top1_med_acc = torch.sum(res[mask & med_confidence_sorted_conf_mask]) / \
                   res[mask & med_confidence_sorted_conf_mask].shape[0]
    top1_high_acc = torch.sum(res[mask & high_confidence_sorted_conf_mask]) / \
                    res[mask & high_confidence_sorted_conf_mask].shape[0]
