...
def get_dataset(seed_num, dataset, split, data_format, low_label_test):
    ...
    if low_label_test > 0:
        new_train_masks = []
        new_val_masks = []
        low_label_split = LabelPerClassSplit(num_labels_per_class=low_label_test,
                                             num_valid=low_label_test * num_of_class, inside_old_mask=True)
...
def get_tf_idf_by_texts(texts, known_mask, test_mask, max_features=1433, use_tokenizer=False):
    ...
    if use_tokenizer:
        ...
        tf_idf_vec = TfidfVectorizer(analyzer="word", max_features=500,
                                     tokenizer=lambda x: tokenizer.tokenize(x, max_length=512, truncation=True))
...
def citeseer_to_graph(citeseer_path='/egr/research-dselab/chenzh85/toy_experiments/ogb/raw_data/CiteSeer-Orig',
                      split="public", embedding_type="original"):
    data = Data()
    ...
    if split != "public":
        ...
        data.test_masks = [test_mask]
    torch.save(data,
               f"/egr/research-dselab/chenzh85/toy_experiments/ogb/preprocessed_data/new/citeseer2_{split}_{embedding_type}.pt")
    return data
